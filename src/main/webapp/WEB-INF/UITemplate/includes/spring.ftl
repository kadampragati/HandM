 ...
 * messsageVar
 *
 * Macro to translate a message code into a message and assign it into variable for later use
-->
<#macro messageVar code>
	<#assign messageText = springMacroRequestContext.getMessage(code) />
</#macro>
...
<#macro formInput path attributes="" fieldType="text">
    <@bind path/>
    <input type="${fieldType}" id="${status.expression?replace('[','')?replace(']','')}" name="${status.expression}" value="<#if fieldType!="password">${stringStatusValue?html}</#if>" ${attributes}<@closeTag/>
</#macro>

<#--
 * displayReadonlyValue
 *
 * Display a form input field of type 'label' and bind it to an attribute
 * of a command or bean.
 *
 * @param path the name of the field to bind to
 * @param attributes any additional attributes for the element (such as class
 *    or CSS styles or size
-->
<#macro displayReadonlyValue path attributes="">
	<@bind path/>
	${stringStatusValue}
</#macro>
 ...
 * formFieldLabel
 *
 * Display the label for the form input field. It checks if there are any form errors
 * for the field, if yes, displays label with different style.
 *
 * @param path the name of the field to bind to
 * @param fieldLabel - label for the field
-->
<#macro formFieldLabel path fieldLabel>
	<@bind path/>
	<label for='${fieldLabel?lower_case?replace(' ', '_')}'
...
<#-- ============== Custome Macros Start ==================== -->
<#macro fieldLabel path fieldLabel label>
	<@bind path/>
	<label for='${label}'
	<#if spring.status.error>
		<#--class='error-text'-->
...
<#macro assignVariableOnError path var>
	<@bind path/>
	<#if spring.status.error>
...
<#macro fieldErrorGlyphicon path attributes="">
	<@bind path/>
	<#if spring.status.error>
		<span class="glyphicon glyphicon-remove form-control-feedback" ${attributes}></span>
	</#if>
</#macro>

<#macro showFieldErrors path separator classOrStyle="">
	<@bind path/>    
 ...
 * @param fieldLabel - label for the field
-->
<#macro customFormFieldLabel path fieldLabel class>
	<@bind path/>
	<label for='${fieldLabel?lower_case?replace(' ', '_')}'
	<#if spring.status.error>
		class='${class}'
...
<#macro formTextarea path attributes="">
    <@bind path/>
    <textarea id="${status.expression?replace('[','')?replace(']','')}" name="${status.expression}" ${attributes}>${stringStatusValue?html}</textarea>
</#macro>
...
<#macro memberFormCheckboxes path options separator attributes="">
    <@bind path/>
    <table>
    <#list options?keys as value>
    <#assign id="${status.expression?replace('[','')?replace(']','')}${value_index}">
